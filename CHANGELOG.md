...
- **ci.yml 六处（D1/DD2/D3/D4）**：check job 加 `timeout-minutes`（job 60 / test 步 45——6h 挂死止损，历史 3 次实证）+ **check-macos job 移除**（D3：macOS `--no-default-features` B 路径实证否决，74 错误 ~71 个非 feature 相关，job 11/11 从未绿；`--no-default-features` 卫生已由 check job Windows build 步覆盖）+ **msrv job ubuntu→windows + `@1.85.0`→`@1.88.0`**（D2 双修）+ audit job **permissions 块**（`contents: read` + `checks: write`——action 建 check-run 所需）+ `ignore: RUSTSEC-2026-0189`（action.yml 实证支持）
- **CI 测试策略实装（D1）**：`src/security/signature.rs::verify_signature_with_policy` 入口 choke-point gate（`CI=true → return Unknown`——与本地非提权行为一致，单点覆盖 WinVerifyTrust 整类挂点：lib `real_path_does_not_panic`（实证挂死 6h）+ `test_signature.rs:497` + scorer big_request 200 synthetic exe 间接路径；GitHub 自动设 CI、本地不设故**本地行为零变化**——开工实测本地 `CI` 为空）+ `tests/test_dns_etw.rs` 3 测试 / `tests/test_schannel_etw.rs` 2 测试 CI skip（`ci_env()` helper——ETW/真实网络类，降级守卫在提权 runner 上失效；纯契约测试保留 CI 覆盖）
- **msrv B 方案（D2）**：`rust-version` 1.85→1.88（lockfile 16 包要求 1.86-1.88，1.85 声明已失真；A 方案 16 包 pin 清单留档 stage-1 附录 C）+ 本地 `rustup toolchain install 1.88.0`（static.rust-lang.org 直连实测 2.6KB/s 不可用，**rsproxy 镜像 4.2MB/s 19 秒装成**）+ `cargo +1.88.0 check --no-default-features` 实证通过（34.6s）。**两项实施发现**：① 1.88 声明解锁 clippy let-chain 版 `collapsible_if`，37 处既有代码（16 文件，本 cycle 未触碰路径）打红——`Cargo.toml [lints.clippy] collapsible_if = "allow"` 过渡 + tech-debt 归档 v0.28+ let-chain 现代化（CI 修复专项不动业务代码）；② 原 msrv 步骤 `--all-targets` 变体在任何工具链编不过测试目标（`test_agent_v0_20_stage_2`/`_stage_3b` 引用 default feature 门控模块，ubuntu 时代死于依赖解析从未暴露）——步骤修正为 `cargo check --no-default-features`（lib+bins，ADR-0037 D2 实装注记）
- **audit 三修（D4）**：permissions 块 + `cargo update -p crossbeam-epoch` 0.9.18→0.9.20（RUSTSEC-2026-0204 patched，lockfile 仅一包变更）+ RUSTSEC-2026-0189 ignore（rmcp 0.11 SSE Host-header：advisory 原文「stdio 不受影响」+ SSE 三条件窄暴露——ADR-0037 D4/附录 E；**TD-63 新立**追踪 0.11→≥1.4 major）。**本地预验**：`cargo audit --ignore RUSTSEC-2026-0189` exit 0（DB 当日拉取；唯一 vulnerability 即 rmcp、crossbeam 已清、余 6 个 warning 级不挡 job）
- **miri/winget 移除（D5 声明性降级）**：`.github/workflows/miri.yml` 整体删除（三重论据：平台不可修 14/14 全红 / 198 unsafe 全在 NT FFI 层 / 目标测试 15 处 sleep 时序断言不适合解释器；`docs/unsafe-audit.md` 两处措辞同步「已移除（ADR-0037 D5）」）+ `release.yml` update-winget job 删除（上游 action 删库 404 + winget-pkgs 无 `Alfroul.proc` manifest——分发从未存在，零用户破坏；实际渠道 = GitHub Releases + cargo-binstall）
- **搭车**：ADR-0003 幽灵引用 **8 处清零**（REVIEW-v0.26 Findings ② 四处：app.rs:181 / CONTEXT.md PID 词条 / ADR-0021 Related（含死链）/ ADR-0036 D4① + 全仓 grep 同类四处：mcp/handler:362 / net_flow:14+35 / score.rs:221——全部改代码层锚 `docs/architecture-deep-dive.md` 条目①；smartctl 语境合法引用与冻结历史文档保留，grep 验收锚通过）+ **TD-63 新立** + TD 台账 v0.27 追踪段 + **README 口径同步**（亮点条删 miri + ADR 36→37 / TD 61→62 + **winget/scoop 安装指引勘误**（winget-pkgs 无 manifest + 本 stage 核实 scoop Main/Extras bucket 均无 proc、作者无自建 bucket——失实指引改 GitHub Releases / binstall）+ MCP SSE 段 RUSTSEC-2026-0189 如实注记（D4「ignore 期间安全叙事如实注记」））
- 回归双档 **1744 / 0 / 10 + 1768 / 0 / 11 与开工基线一致**（各 80 行核对；env-gate 本地零激活 + crossbeam patch 变更不改行为面）；fmt / clippy 双档 / build 双档 / bench --no-run 全过；`cargo +1.88.0 check --no-default-features` 绿；MCP tool 46 / agent catalog 47 / runtime deps +0（Cargo.toml 仅 rust-version + [lints]）不变锚全程保持；开工双档各一起首跑 flaky（默认档 TD-62 本体第 2 次触发 + anthropic 档 `test_inspect::inspect_aggregates_three_buckets` 首触发——均单跑绿 + 复跑绿，观察项口径归档 tech-debt v0.27 段）；远端验证（push → 三 workflow 观察）属 stage 3，push 为用户拍板项
...
v0.26 是秋招窗口期（2026-08~11）的**展示冲刺轻 cycle**（brainstorm 5 决策 2026-08-31 拍板；净 ~2200 行级 doc 为主，**零挂机**）——**「工程强度已足（65K src / 27K tests / 36 ADR / 25 releases），短板在评估者 10 分钟内看不到；把强度从 docs 深处搬到评估者可见处，把纸面质量门禁变成机器强制，补测试深度叙事」**。六组打包全交付：① **README 评估者视图**（mermaid 分层架构图 + 量化亮点条 + 竞品对比表 as-of 2026-08-31 GitHub API 核实 + badge/GIF 注释占位）② **AI Agent 与 eval 纪律章**（六列实验矩阵 32/—/33/35/37/27 + 方差带 ±3±6 标尺 + 预登记换默认门槛 ≥+7 且 L2 方向性 + 负结果三连归档 + L2 5% 能力边界）③ **质量门禁自动化**（R1 llama e2e flaky 根治 + `scripts/gate.sh` 两档（快档稳态 29s）+ pre-push hook opt-in + required checks 设置说明——R2「变更落在人工验证之后」结构性关闭）④ **性能叙事**（`docs/performance.md` 25 数据点 vs v0.13 同机全无回归 + refresh_heavy 2.9× 溯源 TD-47 + 启动/内存实测）⑤ **设计深挖导览**（`docs/architecture-deep-dive.md` 10 条 × 三层深挖 + 证据链接，中性命名）⑥ **测试深度**（proptest FilterExpr roundtrip 属性测试 ×2 + `docs/unsafe-audit.md` + 4 处 SAFETY invariant 注释）。**MCP tool 46 / agent catalog 47 / runtime deps +0 三不变锚全程保持**（dev-deps +1 proptest 为 ADR-0036 D3 预登记批准）。Review 抽查 19 项（> 10 门槛）：**零自创数字**，抓出并修正 1 处源文档算术错误（RAG 引用率 57% → 8/15 = 53%，v0.24 归档表格与汇总行自相矛盾被忠实转写）+ gate 拦截实证（fmt 违规注入 → exit 1）+ ADR-0003 幽灵引用发现（PID 复用键控被引到从未入库的文件，四处引用点修正留 v0.27）。新立 TD-62（session confirm 中断测试负载敏感 flaky 观察项，首触发本 cycle 开工基线）。详见 [REVIEW-v0.26](docs/reviews/REVIEW-v0.26.md)。

### v0.26 stage 4 — Slice C + Review/收尾：深挖导览 + unsafe 审计 + REVIEW-v0.26 + cycle 完结

- **Added**: `docs/architecture-deep-dive.md`（新，ADR-0036 D4 十条封闭兑现——**10 条全保无砍单**，每条 L1/L2/L3 三层深挖预期 + 证据文件链接 + 一句话结论：① PID 复用键控（**证据源修正**：D4 表引 ADR-0003 经 git 全历史查证是幽灵引用，改用代码层五处 `(pid, start_time)` 键控证据 + `process_cache` 单键权衡的诚实边界）② 采集三路 + TD-53 改道三理由 ③ worker 退避 + TD-24 止损 ④ 延迟创建 vs Drop 三缺陷 ⑤ VT100 vs UiFrame 双格式 ⑥ 安全纵深四件套（18 项评分溯源：v0.6 14 → v0.7 R15 → v0.11 R16/17/18）⑦ MCP 46 不变锚 + x-deprecated 双轨 + snapshot 复用 ⑧ OpenAI tools over llama.cpp + GBNF 否决证据链（12 请求 400）⑨ RAG 全链 + 判读纪律 ⑩ Windows-only 权衡 + eval 方法论）
- **Added**: `docs/unsafe-audit.md`（新 ~120 行）——198 处 unsafe（block 191 + fn 2 + extern fn 5）分布判读（FFI 边界层集中是领域属性，业务逻辑层零 unsafe）/ SAFETY 注释 59 → 63 处覆盖分析（ETW + security 组 80-100% vs NT API 调用层零覆盖的优先级解释）/ edition 2024 `unsafe_op_in_unsafe_fn` 已合规结论 / miri 防线如实声明不可用（workflow 待修，v0.27 候选）；NT API 层其余 ~93 处留 v0.27+ 专项（注释写错比没有更伤）
- **Changed**: **4 处 SAFETY 注释小修**（`src/inspect/handles.rs` ×2 + `src/estats.rs` + `src/collect.rs`——动态加载 NT 函数调用 / 灵活数组成员 / 两段式 sizing / ToolHelp 迭代四个模式的 invariant 声明；纯注释零行为变化，回归双档数字不变）
- **Changed**: **CI clippy 1.98 漂移 2 处搭车修**（用户拍板 2026-09-01）——`src/security/hash_cache.rs` match → `?`（question_mark）+ `src/smart/mod.rs` and_then → filter（manual_filter）；本地 1.95 不报此 lint，按远端 CI stable 标准写法机械修复、语义零变化（本地 clippy 双档仍绿）
- **Fixed**: README **三处 RAG 引用率算术错误**——「引用 57%」→「8/15 = 53%」（v0.24 归档表格 8/15 与汇总行 57% 自相矛盾（8/14 才 = 57%），stage 3 忠实转写汇总行，Review 抽查复算抓出；归档原文冻结不改，勘误口径入 REVIEW-v0.26 Findings ①）+ ADR-0027 既有死链（URL 缺 `rmcp-` 前缀）+ 亮点条规模数字更新为收尾态实测（**65,099 src / 27,480 tests**，as-of 2026-09-01 注记——附录 F 时点值 65,074/26,968 已随 stage 2 代码演进漂移）
- **Docs**: `docs/reviews/REVIEW-v0.26.md`（新——**19 项数字溯源抽查**（18 项零自创 + 1 项源文档算术错误）+ gate 有效性实证（fmt 违规注入 → `[1/4]` 步 Diff 检出 → **exit 1** → 还原复绿）+ 展示层链接核验（修复深挖/audit `../` 前缀错误 + README 死链）+ 深挖导览按 stage 产出同标准审 + Findings P0 0 / P1 0 / P2 4 + §6 v0.27 候选评估）+ TD 回填（**TD-62 新立** + v0.26 追踪段 stage 4 行 + 历史回顾）+ ADR-0036 stage 4 实装注记 + CONTEXT.md（深挖导览落地态 / 幽灵引用 / unsafe-audit 三词条）+ README v0.26 banner + **demo GIF 挂载**（`docs/assets/demo.gif` 1.34 MB / 35.1s，ShareX 采集：TUI 主界面 → AI Agent 面板一次 tool-use 完整链路，README 注释占位替换）+ CHANGELOG 0.26.0 + Cargo.toml `0.25.0` → `0.26.0` + tag `v0.26.0` + push origin/master（用户拍板）
- 回归双档终值 **1744 / 0 / 10 + 1768 / 0 / 11**（各 80 行核对；stage 4 零行为变化——注释 + 机械改写，与 stage 2/3 终值一致）；开工基线 anthropic 档曾连续 2 次红在同一断言（`test_session_drop_during_confirm_does_not_hang` 负载敏感：复跑绿 + 单跑绿 + 机器后台重负载实测归因 → TD-62 观察项，不在 gate 快档名单）；fmt / clippy 双档 / build 双档 / bench --no-run 全过
