...
fn tab_index(mode: &AppMode) -> usize {
    match mode {
        AppMode::ProcessList | AppMode::ProcessDetail => 0,
        AppMode::ProcessTree => 1,
        AppMode::PortMap => 2,
...
fn draw_toolbar(f: &mut Frame, app: &App, area: Rect) {
    ...
    let tabs = Tabs::new(titles)
        .block(Block::default().borders(Borders::BOTTOM).style(theme::style_header()))
        .select(active_tab);
...
fn draw_middle(f: &mut Frame, app: &App, area: Rect) {
    match app.mode {
        AppMode::ProcessList | AppMode::ProcessTree | AppMode::Replay => {
...
fn draw_main_panel(f: &mut Frame, app: &App, area: Rect) {
    match app.mode {
        ...
        AppMode::Help => {
            let text = "帮助\n\n按 1-6 切换面板\n按 q 退出\n按 Esc 返回".to_string();
            let p = Paragraph::new(text).style(theme::style_normal());
...
fn draw_footer(f: &mut Frame, app: &App, area: Rect) {
    let base = match app.mode {
        AppMode::UsbAssistant => " ↑↓移动  Enter选择设备  k终止安全进程  r刷新  w持续监测  Tab切换设备  q退出",
        AppMode::MonitorPanel => " ↑↓移动  a添加监控  d删除  r重启  s暂停/恢复  q退出",
        AppMode::DockerPanel => " ↑↓移动  Enter详情  r重启  s停止  a监听事件  q退出",
        AppMode::PortMap => " ↑↓移动  Enter展开/详情  g切换视图  a异常  f过滤  s排序  d诊断  /搜索  k终止  q退出",
        AppMode::Replay => " Space播放/暂停  ←→快退/快进  +/-速度  q退出回放",
        _ => " ↑↓移动  Space选择  Enter操作  /搜索  S安全排序  q退出",
    };
    ...
    let help = Paragraph::new(format!("{}{}{}", msg, kill_msg, theme_label)).style(theme::style_muted());
