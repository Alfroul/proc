...
进程列表/树中按 `Enter` 进入详情页，顶部 4 个 Tab 切换深挖视图（v0.4.0，ADR-0004）：

- **概要**：原详情页内容（CPU/内存/磁盘/运行时长/exe/cmd/cwd/端口/安全分/风险因子）
- **环境**：进程环境变量，`/` 搜索过滤，↑↓/PgUp/PgDn 滚动
- **网络**：该 PID 的所有监听与连接（TCP/UDP + 远程地址 + 状态）
- **DLL**：已加载模块（Windows DLL / Linux `.so`），按路径字母排序，可搜索

Windows 走 PEB walk + ToolHelp 快照；Linux 读 `/proc/<pid>/environ` + `/proc/<pid>/maps`；macOS 等其它平台该 Tab 显示降级提示。
